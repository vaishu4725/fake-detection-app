...
# ── Load ML model ──
MODEL_PATH      =  os.path.join(os.path.dirname(__file__), 'pac_model.pkl')
VECTORIZER_PATH = os.path.join(os.path.dirname(__file__), 'tfidf_vectorizer.pkl')
META_COLS_PATH  = os.path.join(os.path.dirname(__file__), 'meta_features.npy')
...
def load_ml_model():
    ...
    try:
        if os.path.exists(MODEL_PATH):
            ...
            ml_meta_cols = np.load(META_COLS_PATH, allow_pickle=True)
...
def run_fraud_detection(job_dict):
    ...
    try:
        text = job_dict.get('description', '')
        vector = ml_vectorizer.transform([text])

        prediction = ml_model.predict(vector)[0]

        confidence = max(ml_model.predict_proba(vector)[0])
...
@app.route('/')
def home():
    return render_template('truehire.html')
